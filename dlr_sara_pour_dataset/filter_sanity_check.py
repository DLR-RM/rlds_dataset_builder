    ...
    for f in file_list:
        ...
        for i in range(l):
            if i < l - 1:
                ...
                h_msr = h_msr.dot(delta)
                h_msr = np.hstack((h_msr[:3, 3], Rotation.from_matrix(h_msr[:3, :3]).as_euler('zxy')))
                # print(f"step_t[msr]: {h_msr}")
                step_t_1_orig = data_orig[i+1]

                # print(f"step_t_1[state]: {step_t_1_orig['state']}")
                if np.linalg.norm(h_msr - step_t_1_orig['state']) > 0.000000000000005:
                    print(np.linalg.norm(h_msr - step_t_1_orig['state']))
