...
def transform_step(step: Dict[str, Any]) -> Dict[str, Any]:
    ...
    img = Image.fromarray(step["observation"]["image"]).resize((128, 128), Image.Resampling.LANCZOS)
    transformed_step = {
        "observation": {
            "image": np.array(img),
        },
        "action": np.concatenate([step["action"], [np.float32(1)], [step["is_terminal"]]]),
    }

    # copy over all other fields unchanged
    for copy_key in ["discount", "reward", "is_first", "is_last", "is_terminal", "language_instruction", "language_embedding"]:
